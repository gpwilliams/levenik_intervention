...
This section outlines additional analyses that were pre-registered but not reported in the main body of the manuscript. These analyses were not included because they largely echo the results of the (also pre-registered) effects of Variety Exposure condition within Task for novel words only. Essentially, this analyses (collapsing across all word types) and the analysis for novel words only ask whether there are any effects of dialect exposure on general literacy.

## Testing Phase Model

### Simple Effect of Variety Exposure within Each Task (i.e. Collapsing across all Word Types)
...
- Overall performance will be better in the Dialect & Social condition relative to the Dialect condition due to the social cue allowing learners to suppress irrelevant language varieties; i.e. the dialect variety when doing the literacy test in the standard variety. This should lead to less cognitive load and confusion between varieties.
...
Posterior medians with 80% and 90% credible intervals are shown collapsing across all words within each task and variety exposure condition below.

```{r task-variety-testing-plot}
knitr::include_graphics(plots_dir[str_detect(plots_dir, "testing_tv.png")])
```

Here, the most consistent effect is that performance is generally worse on the spelling task relative to the reading task. Posterior medians with 80% and 90% credible intervals are shown for the comparison between each Variety Exposure condition within each Task across all word types below.
