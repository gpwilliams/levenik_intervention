...
base_plot <- ggplot(
  data = escape_character(draws$exposure_v_compare_agg, variety_exposure),
...
plots$exposure_v_compare_agg <- base_plot +
  labs(
...
plots$exposure_vw_compare_agg <- base_plot %+%
  draws$exposure_vw_compare_agg +
  labs(
    x = plotting_options$diff_nLED_title, 
    y = NULL,
    title = paste0(
...
plots$testing_v_compare_agg <- base_plot %+%
  escape_character(draws$testing_v_compare_agg, variety_exposure) +
  labs(
    x = plotting_options$diff_nLED_title, 
    y = NULL,
    title = paste0(
...
plots$testing_v_n_compare_agg <- base_plot %+% 
  escape_character(draws$testing_v_n_compare_agg, variety_exposure)  +
  labs(
    x = plotting_options$diff_nLED_title, 
    y = NULL,
    title = paste0(
...
plots$testing_tvw_compare_agg <- base_plot %+% 
  draws$testing_tvw_compare_agg +
  facet_grid(.~task) +
  labs(
    x = plotting_options$diff_nLED_title, 
    y = NULL,
...
plots$testing_tvw_ms_compare_agg <- draws$testing_tvw_ms_compare_agg %>% 
...
plots$testing_cov_median_etv_n_compare_agg <- ggplot(
  data = escape_character(draws$testing_cov_median_etv_n_compare_agg, variety_exposure),
  aes(y = variety_exposure, x = .value)
) +
  geom_halfeyeh(
    fill = plotting_options$fill, 
...
plots$testing_cov_median_etvw_compare_agg <- ggplot(
