...
model_summaries$exposure_v_compare_agg <- gmap_rope(
...
model_summaries$exposure_vw_compare_agg <- gmap_rope(
  data = draws$exposure_vw_compare_agg, 
  draws = .value, 
  variety_exposure,
  word_type,
...
# difference in nLED between variety exposure conditions
model_summaries$testing_v_compare_agg <- gmap_rope(
  data = draws$testing_v_compare_agg, 
  draws = .value, 
  variety_exposure,
  bounds = c(-.035, .035)
) %>% 
...
model_summaries$testing_v_n_compare_agg <- gmap_rope(
  data = draws$testing_v_n_compare_agg, 
  draws = .value, 
  variety_exposure,
  bounds = c(-.035, .035)
) %>% 
...
model_summaries$testing_tvw_compare_rope_agg <- gmap_rope(
  data = draws$testing_tvw_compare_agg, 
  draws = .value, 
  task, 
  variety_exposure,
  word_type,
...
model_summaries$testing_tvw_ms_compare_agg <- gmap_rope(
  data = draws$testing_tvw_ms_compare_agg, 
  draws = .value, 
  task, 
  word_type,
  variety_exposure,
...
model_summaries$testing_cov_median_etv_n_compare_agg <- gmap_rope(
  data = draws$testing_cov_median_etv_n_compare_agg, 
  draws = .value, 
...
model_summaries$testing_cov_median_etvw_compare_agg <- gmap_rope(
  data = draws$testing_cov_median_etvw_compare_agg, 
  draws = .value, 
  exposure_test_nLED_group,
  task, 
  variety_exposure,
