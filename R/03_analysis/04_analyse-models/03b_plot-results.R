...
plots$exposure_v_agg <- ggplot(
  data = escape_character(
    draws$exposure_v_agg, 
    variety_exposure, 
    pattern = " "
  ), 
...
plots$testing_tvw_agg <- ggplot(
  data = escape_character(draws$testing_tvw_agg, word_type, after = TRUE), 
  aes(x = .value, y = word_type)) +
  geom_halfeyeh(
    fill = plotting_options$fill, 
    color = plotting_options$colour,
...
plots$testing_tv_n_agg <- ggplot(
  data = draws$testing_tvw_agg %>% 
...
plots$testing_cov_etv_agg <- ggplot(
  data = draws$testing_cov_tvw_agg %>% 
    escape_character(variety_exposure, pattern = " "), 
...
plots$testing_cov_etv_n_median_agg <- ggplot(
  data = draws$testing_cov_median_etv_agg %>% 
    filter(word_familiarity == "Novel") %>% 
    escape_character(variety_exposure, pattern = " "), 
...
plots$testing_cov_etvw_median_agg <- ggplot(
  data = draws$testing_cov_median_etv_agg %>% 
    filter(word_familiarity != "Novel") %>% 
    escape_character(variety_exposure, pattern = " ") %>% 
