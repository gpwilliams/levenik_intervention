...
```{r exposure-model-summary-csv, include = FALSE}
exposure_mod <- fitted_models[["exposure_model_summary"]]$fixed %>%
  as.data.frame() %>% 
  ...
  mutate(
    Parameter = str_replace_all(Parameter, "\\$\\\\times\\$", "x"),
...
write_csv(
  exposure_mod,
  here("05_reports", "01_model-coefficients", "exposure_model.csv")
)
```
...
```{r testing-model-summary-csv, include = FALSE}
testing_mod <- fitted_models[["testing_model_summary"]]$fixed %>%
  as.data.frame() %>% 
  rownames_to_column(., var = "Parameter") %>% 
  merge_CI_limits(., `l-95% CI`, `u-95% CI`, decimals = 2) %>%
  select(Parameter:Est.Error, interval, everything()) %>% 
...
write_csv(
  testing_mod,
  here("05_reports", "01_model-coefficients", "testing_model.csv")
)
```
...
```{r testing-cov-model-summary-csv, include = FALSE}
testing_cov_mod <- fitted_models[["testing_cov_model_summary"]]$fixed %>%
  as.data.frame() %>% 
  rownames_to_column(., var = "Parameter") %>% 
  merge_CI_limits(., `l-95% CI`, `u-95% CI`, decimals = 2) %>%
  select(Parameter:Est.Error, interval, everything()) %>% 
...
write_csv(
  testing_cov_mod,
  here("05_reports", "01_model-coefficients", "testing_cov_model.csv")
)
```
